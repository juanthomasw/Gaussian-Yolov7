...
class YOLOLayer(nn.Module):
    ...
    def __init__(
        ...
        ### END OF GAUSSIAN MODEL -> self.no = 5 + self.nc + uncertainties = 10
    ...
    def forward(self, xin, labels=None):
        ...
        output = output.permute(0, 1, 3, 4, 2)  # shape: [bs, na, ny, nx, no]

        ### GAUSSIAN MODEL
        
        if self.gaussian:
            # logistic activation for sigma of xywh
            sigma_xywh = output[..., -4:]  # shape: [bs, na, ny, nx, 4(xywh uncertainties)]
            sigma_xywh = torch.sigmoid(sigma_xywh) # sigmoid activation -> value 0-1

            output = output[..., :-4] # shape: [bs, na, ny, nx, n_class + 5(= x, y, w, h, objectness)] -> back to original model

        ### END OF GAUSSIAN MODEL
        
        # logistic activation for xy, obj, cls -> value 0-1
        ...
        # From grid cells to bounding boxes of YOLOv3
        pred = output.clone()
        #  Transform the output coordinates from being w.r.t. grid cells to being w.r.t. the image (bounding boxes)
        pred[..., 0] += x_shift # b_x = sigmoid (x) + x_shift
        pred[..., 1] += y_shift # b_y = sigmoid (y) + y_shift
        # Integrate the output of the model with anchors by ensuring if w & h = 0, default anchors are used
        pred[..., 2] = torch.exp(pred[..., 2]) * w_anchors # b_w = w_anchors * exp (w)
        pred[..., 3] = torch.exp(pred[..., 3]) * h_anchors # b_h = h_anchors * exp (h)

        if labels is None:  # not training -> inference
            pred[..., :4] *= self.stride
            pred = pred.view(batchsize, -1, n_ch)  # shape: [bs, na * ny * nx, nc + 5]

            ### GAUSSIAN MODEL

            if self.gaussian:
                # scale objectness confidence with xywh uncertainties
                sigma_xywh = sigma_xywh.view(batchsize, -1, 4)  # shape: [bs, na * ny * nx, 4]
                sigma = sigma_xywh.mean(dim=-1) # mean of xywh uncertainties -> shape: [bs, na * ny * nx]
                pred[..., 4] *= (1.0 - sigma) # element-wise multiplication with (1-sigma) 
                ...
                pred = torch.cat([pred, sigma_xywh], 2)  # shape: [bs, na * ny * nx, nc + 5 + uncertainties]

            return pred.data

        pred = pred[..., :4].data  # shape: [bs, na, ny, nx, 4(= x, y, w, h)]

        # target assignment

        tgt_mask = torch.zeros(batchsize, self.n_anchors,
                               fsize, fsize, 4 + self.n_classes).type(dtype) # shape: [bs, na, ny, nx, nc + 4]
        obj_mask = torch.ones(batchsize, self.n_anchors,
                              fsize, fsize).type(dtype) # shape: [bs, na, ny, nx]
        tgt_scale = torch.zeros(batchsize, self.n_anchors,
                                fsize, fsize, 2).type(dtype) # shape: [bs, na, ny, nx, 2]

        target = torch.zeros(batchsize, self.n_anchors,
                             fsize, fsize, n_ch).type(dtype) # shape: [bs, na, ny, nx, nc]
        ...
        # loss calculation
        output[..., 4] *= obj_mask # obj
        output[..., np.r_[0:4, 5:n_ch]] *= tgt_mask # x, y, w, h, classes

        target[..., 4] *= obj_mask # obj
        target[..., np.r_[0:4, 5:n_ch]] *= tgt_mask # x, y, w, h, classes
        ...
        # GAUSSIAN MODEL - NLL LOSS _> - log (PDF + 1e-9)/2
        ...
        # END OF GAUSSIAN MODEL
            ...
            loss_xy = F.binary_cross_entropy(output[..., :2], target[..., :2], reduction='none') # bce loss
            loss_wh = F.mse_loss(output[..., 2:4], target[..., 2:4], reduction='none') / 2.0 # mse losss
    ...
    def _gaussian_dist_pdf(self, val, mean, var): #PDF
